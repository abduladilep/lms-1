...
class Challenge extends Component {
  ...
  isRole(roleToCheck) {
    const userRole =
      localStorage.getItem("lms") &&
      JSON.parse(localStorage.getItem("lms")).user
        ? JSON.parse(localStorage.getItem("lms")).user.role
        : null;
  ...
  handleScroll(event) {
    ...
    if (
      ...
      if (this.state.countLoad === data.length && this.state.ChallengeView) {
        this.props.actions.todoChallenge.loadChallenge(
  ...
  handleSaveStudentAccess(a) {
    if (this.state.typeAction === "access")
      this.props.actions.todoChallenge.editChallenge(
        a.id,
        a.title,
        a.content,
        a.category,
        a.file,
        JSON.stringify(a.student_access)
      );
  ...
  handleActionForm() {
    ...
    if (this.state.typeAction === "add")
      this.props.actions.todoChallenge.addChallenge(
        title.trim(),
        content.trim(),
        category.trim(),
        file.trim()
  ...
  handleCropOk() {
    if (this.state.cropResult)
      this.setState({
        ...
        editData: {
          ...this.state.editData,
          file: document.getElementById("cropper").src,
        },
      });
  }
  ...
  render() {
    ...
    const id_user =
      localStorage.getItem("lms") &&
      JSON.parse(localStorage.getItem("lms")).user
        ? JSON.parse(localStorage.getItem("lms")).user.id_user
        : null;
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (student_access)
          for (var j = 0; j < student_access.length; j++) {
            if (student_access[j] === id_user) filterData.push(data[i]);
          }
      }
      data = filterData;
    }
    ...
    if (
      localStorage.getItem("lms") &&
      JSON.parse(localStorage.getItem("lms")).user &&
      JSON.parse(localStorage.getItem("lms")).token
    )
      return (
        <div id="wrapper">
          <Header
            isRole={this.isRole.bind(this)}
            actions={this.props.actions.todoAuth}
            data={this.props.state.auth}
          />
          ...
          <div id="page-wrapper">
            ...
            {this.state.ChallengeView && (
              <div className="row">
                <div className="panel panel-default">
                  <div className="panel-heading">Table Challenge</div>
                  <div className="panel-body">
                    ...
                    >
                      ...
                      <Modal.Body>
                          ...
                          <div>
                            ...
                            <div className="form-group">
                              ...
                              <input
                                ...
                                onChange={this.handleUpdateData.bind(this)}
                              />
                                  ...
                                  <div style={{ maxWidth: 280 }}>
                                    {this.state.cropOkResult && (
                                      <div>
                                        ...
                                        <button
                                          className="btn btn-primary"
                                          onClick={this.handleCropOk.bind(this)}
                                        >
                                          Ok Crop
                                        </button>
                                        <button
                                          className="btn btn-danger"
                                          onClick={this.handleCropClear.bind(
                                            this
                                          )}
                                        >
                                          Clear Crop
                              ...
                              {!this.state.cropResult && (
                                <div style={{ maxWidth: 280 }}>
                                  ...
                                  <img
                                    draggable="false"
                                    style={{ width: "100%" }}
                                    src={`http://localhost:3001/upload/get/challenge/${this.state.editData.file}`}
                                    alt="cropped image"
                                    id="cropper"
                                    className="img-thumbnail"
                                  />
                        ...
                        {this.state.typeAction === "delete" && (
                          <p className="help-block">
                            Are you sure delete {this.state.editData.title}
                          </p>
                        )}
                      </Modal.Body>
                      <Modal.Footer>
            ...
            {this.state.ImageView && (
              <ImageViewElement
                handleBackImage={this.handleBackImage.bind(this)}
                data={this.state.editData}
              />
            )}
            {this.state.StudentAccessView && (
              <StudentAccess
                handleActionForm={this.handleActionForm.bind(this)}
                handleSaveStudentAccess={this.handleSaveStudentAccess.bind(
                  this
                )}
                handleBackStudentAccess={this.handleBackStudentAccess.bind(
                  this
                )}
                data={this.state.editData}
                access={this.props.state.student}
    ...
    else return <div id="wrapper"></div>;
  }
}
